...
/** A {@link SqlEmit} that have some extension functions like partition, overwrite. **/
public class SqlEmit extends SqlCall {
	...
	private long getDelayValue(SqlNode delay) {
		if (delay == null) {
			...
			return isWithoutDelay(delay) ? 0L : ((BigDecimal) ((SqlLiteral) delay).getValueAs(
				BigDecimal.class)).longValue();
		}
	}
				...
				if (SqlKind.PRECEDING != s.getKind() || beforeDelay != null) {
					if (SqlKind.FOLLOWING != s.getKind() || afterDelay != null) {
						throw new RuntimeException(
							"Sql Emit statement shouldn't contain duplicate strategies");
					}
	...
	static {
		...
		BEFORE_COMPLETE_OPERATOR = new SqlPostfixOperator(
			"BEFORE COMPLETE",
			SqlKind.PRECEDING,
			20,
			ReturnTypes.ARG0,
		...
		AFTER_COMPLETE_OPERATOR = new SqlPostfixOperator(
			"AFTER COMPLETE",
			SqlKind.FOLLOWING,
			20,
			ReturnTypes.ARG0,
